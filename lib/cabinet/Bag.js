...
class BagQuery {
    ...
    equals(key, value, strict = false) {
        this.constraints.push(new EqualsConstraint(key, value, strict))

        return this
    }

    includes(key, value) {
        this.constraints.push(new IncludesConstraint(key, value))

        return this
    }

    passes(func) {
        this.constraints.push(new FunctionConstraint(func))
        ...
        */
        for (let value of bagMap.values()) {
            ...
            if (this.orderKey == null) {
                ...
                */
                results.push(value)
                if (results.length >= this.itemLimit) break
            } else {
                let sortPos = 0
