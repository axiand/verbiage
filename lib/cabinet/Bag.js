...
const { existsSync, appendFileSync, openSync, writeSync } = require("fs")
...
class Bag {
    ...
    get syncPath() {
        if (!this.syncMode) return null

        return pathModule.join(this.syncDir, this.syncName)
    }
    ...
    set(data) {
        let pkey = data[this.primaryKey]

        if (this.readOnly) throw new Error(`Attempt to set on a read-only Bag`)
        if (pkey == null) throw new Error(`Object to be added does not have the primary key attribute (${this.primaryKey})`)

        this.items.set(pkey, data)
        if (this.syncMode) this.updateSyncRef(pkey)
        return this
    }
    ...
    async updateSyncRef(id) {
        let existingRef = this.syncRefs.get(id)
        let existingItem = this.items.get(id)

        let packedRef = await SyncRef.pack(existingItem)

        if (!existingRef) {
            let newRef = new SyncRef(this.syncOffset, this)
            this.syncOffset += packedRef.length
            this.syncRefs.set(id, newRef)
            existingRef = newRef

            existingRef.write(packedRef)
        }
    }
    ...
    static fromFile(path, options = {}) {
        let bag = new Bag(options)
        if (!path) throw new Error("A path is required to sync a Bag from a file")

        let pathSpec = pathModule.parse(path)
        if (!existsSync(pathSpec.dir)) throw new Error(`Destination dir for bag does not exist: ${pathSpec.dir}`)

        bag.syncMode = true
        bag.syncDir = pathSpec.dir
        bag.syncName = pathSpec.name
        if (!existsSync(bag.syncPath)) {
            appendFileSync(bag.syncPath, Buffer.alloc(0))
        }

        let fd = openSync(bag.syncPath, "r+")
        bag.syncDescriptor = fd
...
class SyncRef {
    ...
    write(data) {
        writeSync(this.owner.syncDescriptor, data, null, null, this.offset)
    }

    constructor(offset, owner) {
        this.offset = offset
        this.owner = owner
        return this
    }

    static async pack(data, active = true) {
        let jstring = JSON.stringify(data)

        let offset = 0
        let buf = Buffer.alloc(jstring.length + 3)
        buf.writeUInt8(active ? 0xFF : 0x00, offset)
        offset++

        buf.write(jstring, offset)
